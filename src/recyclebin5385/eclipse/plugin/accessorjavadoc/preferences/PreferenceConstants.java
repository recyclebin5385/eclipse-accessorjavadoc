package recyclebin5385.eclipse.plugin.accessorjavadoc.preferences;

/**
 * プラグイン設定の定数定義。
 */
public class PreferenceConstants {
    /**
     * クラス変数名の正規表現
     */
    public static final String P_FIELD_NAME_REGEX = "fieldNameRegex";

    /**
     * getterのJavadocの概要のテンプレート
     */
    public static final String P_GETTER_JAVADOC_SUMMARY_TEMPLATE = "getterJavadocSummaryTemplate";

    /**
     * setterのJavadocの概要のテンプレート
     */
    public static final String P_SETTER_JAVADOC_SUMMARY_TEMPLATE = "setterJavadocSummaryTemplate";
}